...
public class GameBoard extends javax.swing.JPanel {
    ...
    private void addMouseListener(GameBoard gameBoard) {
		// TODO Auto-generated method stub
		
	}

	private void addMouseMotionListener(GameBoard gameBoard) {
		// TODO Auto-generated method stub
    ...
    public void update(Graphics g){
        ...
        for (int i=0;i<19;i++){
            drawHexagon(hexagons[i],SettlersConstants.COLORS[colors[i]]);              
  ...
  /*
    ...
    private void drawSettlement(int x, int y,byte settlement){        
        for (int i=0;i<4;i++){
                if (settlement==SettlersConstants.SETTLEMENT[i]){
                    big.setPaint(SettlersConstants.PLAYER_COLORS[i]);
                    big.fillOval(x-7,y-7,14,14);
                    break;
                }
            }
            for (int i=0;i<4;i++){
                if (settlement==SettlersConstants.CITY[i]){
                    big.setPaint(SettlersConstants.PLAYER_COLORS[i]);
    ...
    private void drawRoad(int x0, int y0,int x1, int y1, byte road){   
        ...
        for (int i=0;i<4;i++){
                if (road==SettlersConstants.ROAD[i]){
                    big.setPaint(SettlersConstants.PLAYER_COLORS[i]);
